...
const Favoritos = () => {
    ...
    const availableFavorites = filteredFavorites.filter(
        (f) => f.books.status === 'available' && f.books.available_copies > 0
    );
...
{ favorites.filter(f => f.books.status === 'available' && f.books.available_copies > 0).length }
</div>
...
<Button 
    ...
    () => {
        if (availableFavorites.length > 0) {
            alert(`Solicitar ${availableFavorites.length} empréstimos`);
        }
    }
}
disabled = { availableFavorites.length === 0 }
    >
    Solicitar disponíveis({ availableFavorites.length })
        </Button>
        </div>
        </div>
        </div>
